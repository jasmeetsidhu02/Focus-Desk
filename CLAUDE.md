...
12. **Current step:** Polish pass — hover cursor styling (R3F doesn't do this automatically, needs `document.body.style.cursor` or similar in the pointer handlers), responsive canvas sizing (verify — `<Canvas>` handles resize internally, but confirm at a few breakpoints), reduced-motion consideration (the `InfoPanel` slide transition and any future camera/light animation should respect `prefers-reduced-motion`), mobile/touch check (hover-based glow may not behave well on touch — see earlier discussion), and remove the debug `PointLightHelper` once lighting is tuned.
13. Deploy (Vercel/Netlify) + push to GitHub with a clean README + add live link to portfolio and resume.
